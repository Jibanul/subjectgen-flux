...
def parse_args(input_args=None):
    ...
    # parser.add_argument("--general_lora_weights", type=str, required=True) #J:
...
def finetune_lora(args, models=None):
    # Load model weights
    vae = models["vae"]
    ...
    dino = models["dino"]
    transforms_configs = models["transforms_configs"]
    ir_feature_extractor = models["ir_feature_extractor"]
    feature_extractor_transforms = models["feature_extractor_transforms"]
    ...
    # pipeline.load_lora_weights(args.general_lora_weights)

    # Freeze all non-LoRA parameters and ensure LoRA parameters are trainable
    for name, param in transformer.named_parameters():
        if "lora" not in name:  # Freeze non-LoRA transformer parameters
            param.requires_grad = False
        else:  # LoRA parameters should remain trainable
            param.requires_grad = True

    # If you're training the text encoder, do the same for it:
    if args.train_text_encoder:
        for name, param in text_encoder_one.named_parameters():
            if "lora" not in name:  # Freeze non-LoRA text encoder parameters
                param.requires_grad = False
            else:  # LoRA parameters should remain trainable
                param.requires_grad = True

    # # Print out the trainable parameters to check
    ...
    p for n, p in transformer.named_parameters() if "lora" in n and p.requires_grad
    ]

    # Prepare params for optimizer (only LoRA parameters)
    params_to_optimize = [{"params": transformer_lora_params, "lr": args.learning_rate}]
    if args.train_text_encoder:
        text_lora_params = [
            p for n, p in text_encoder_one.named_parameters() if "lora" in n and p.requires_grad
        ]
        params_to_optimize.append({"params": text_lora_params, "lr": args.text_encoder_lr if args.text_encoder_lr else args.learning_rate})
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        # print("args.prompt.split('.')[0]", args.prompt.split('.')[0])
        # print("args.prompt", args.prompt)
        with accelerator.accumulate(models_to_accumulate):
            image = (
                pipeline(
                    # prompt=args.prompt.split('.')[0],
                    prompt=args.prompt,
            ...
            if loss < best_loss:
                ...
                if args.save_weights and accelerator.is_main_process:
                    ...
                    #J:
                    # print("Available PEFT adapter names in transformer:", best_transformer.peft_config.keys())

                    best_transformer_lora_layers = get_peft_model_state_dict(
                        best_transformer, adapter_name="default_0"
                    )

                    if args.train_text_encoder:
                        best_text_encoder_one = unwrap_model(text_encoder_one)
                        # print("Available PEFT adapter names in text encoder:", best_text_encoder_one.peft_config.keys())
                        best_text_encoder_lora_layers = get_peft_model_state_dict(
                            best_text_encoder_one, adapter_name="default_0"
                        )
                    else:
...
def generate_img(args, pipeline_inference):
    #J: backend
    ## Inference using the target prompt
    # device = torch.device(f"cuda:{args.device_id}")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # pipeline_inference = FluxPipeline.from_pretrained(args.pretrained_model_name_or_path, torch_dtype=torch.bfloat16,).to(device)
    

    # Now load subject-specific LoRA on top
    subject_lora_weights_path = os.path.join(args.weights_output_dir, "pytorch_lora_weights.safetensors")
    # print("subject_lora_weights_path", subject_lora_weights_path)
    pipeline_inference.load_lora_weights(subject_lora_weights_path)
    
    # target_prompt = '. '.join([s.strip() for s in args.prompt.split('.') if s.strip()][1:]) + '.'
    #J: backend
...
if __name__ == "__main__":
    ...
    models = load_models()
    finetune_lora(args, models)

    pipeline_inference = load_inference_pipeline()
    generate_img(args, pipeline_inference=pipeline_inference)
